    ...
    user.comparePassword(req.body.password, (err, isMatch) => {
        if (err) return errbody(res, "Error")
            ...
            user.generateToken((err, { token }) => {
                return respbody(res, { token, user })
            })
        }
    })
})
...
router.get("/users", async (req, res) => {
    try {
        let users = await User.find({}).select('_id, mail');
        if (!users) {
            return errbody(res, "No users")
        }
        return respbody(res, users.map(user => ({ key: user._id, mail: user.mail })))
...
router.get("/user", async (req, res) => {
    try {
        let user = await User.findOne({ _id: req.query._id }).select('_id, mail');
        if (!user) {
            return errbody(res, "No user")
        }
        return respbody(res, user)
